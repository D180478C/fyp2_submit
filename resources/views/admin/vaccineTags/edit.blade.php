...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.vaccineTag.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.vaccine-tags.update", [$vaccineTag->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('vaccine_status') ? 'is-invalid' : '' }}" name="vaccine_status" id="vaccine_status" required>
                    ...
                    @foreach(App\Models\VaccineTag::VACCINE_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('vaccine_status', $vaccineTag->vaccine_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
